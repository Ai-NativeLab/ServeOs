...
| # | Decision | Choice |
|---|----------|--------|
| D1 | Audit log | **Append-only, hash-chained (tamper-evident) AND device/session fingerprinted.** Every action carries `prev_hash`/`entry_hash` plus a `{deviceId, appVersion, ip, userAgent}` fingerprint. |
