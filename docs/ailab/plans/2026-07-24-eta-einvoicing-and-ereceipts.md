...
> **For agentic workers:** REQUIRED SUB-SKILL: Use mo-ai:subagent-driven-development to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking. **`## Blocked — VERIFY with ETA` gates the wire format** — do not finalise `EtaFiscalProvider.submit`'s request body or `qrPayload`/signature encoding until those items are confirmed against the live ETA developer portal.
...
```bash
git add src/server/fiscal/build-document.ts src/server/fiscal/eta-provider.ts src/server/fiscal/config.ts src/server/fiscal/build-document.test.ts src/server/fiscal/eta-provider.test.ts
git commit -m "feat(fiscal): pure buildReceipt/buildCreditNote mappers + EtaFiscalProvider (wire format TODO(VERIFY))"
```
...
**These items block finalising `EtaFiscalProvider.submit`'s wire format, the signing path, and the QR/credit-note rendering.** They are **not** confirmed by public research (see the spec's "Compliance items to VERIFY with ETA"). Do **not** invent ETA field names, endpoints, or encodings — build the surrounding machinery (Tasks 1–7) against the typed `FiscalDocument`/`FiscalSubmitResult` contract, keep the ETA-facing specifics behind `TODO(VERIFY)` markers, and only wire the real request/response body once each item below is confirmed against the current ETA developer portal / SDK. Each is load-bearing for legality.
