...
- [x] **Step 7: Run tests + typecheck + lint.** `npx vitest run src/server/rbac/permissions.test.ts src/server/fiscal/config-service.test.ts && npx tsc --noEmit && npx eslint src/server/fiscal src/server/rbac src/app/api/dashboard/fiscal src/app/api/pos/v1/sales src/app/dashboard/fiscal`. Expected: PASS, clean.
  ...
  **Counts, corrected in spec review and re-measured rather than estimated.** This task touches **5 test files** — `config-service.test.ts` (new, 23), `fiscal-routes.test.ts` (new, 7), `api/fiscal/worker/route.test.ts` (new, 3), `permissions.test.ts` (14 → 16, +2) and `nav-items.test.ts` (8 → 9, +1) — for **36 new tests**. Scope run (`src/server/{fiscal,audit,rbac}` + `src/app` + `src/components`): **45 files / 517 tests passed**. Full `npm test`: **189 files / 1534 tests passed**. An earlier draft of this note said 1532; that figure predated two tests landing (`redactedCause`, then the extended masking walk) and understated the suite. `npx tsc --noEmit` clean. `npx eslint src/server/fiscal src/server/rbac src/app src/components/dashboard` reports only 6 **pre-existing** `no-html-link-for-pages` errors in `admin/`, `login/` and `register/` pages, none of them touched here; nothing new. `npx next build` succeeds with all seven new paths registered as dynamic server functions, which is the check that would have caught a route conflict.
