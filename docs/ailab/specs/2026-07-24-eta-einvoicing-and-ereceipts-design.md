...
An Egyptian POS that does not submit to ETA is illegal to operate for any mandated taxpayer. The B2C e-receipt mandate binds taxpayers **named in cumulative ETA wave decisions** (e.g. Decision 281/2025, 8th stage sub-phase 2, effective 15 Sep 2025; latest verified wave **361/2025**, effective 15 Nov 2025), while B2B e-invoicing already binds **all** registered taxpayers (Decision 323/2022; ETA FAQ). The once-reported "threshold cut to EGP 250,000 / register by 31 Mar 2026" is **contradicted by official sources** — ETA's FAQ still states the EGP 500,000 VAT threshold. Adoption is forced economically regardless: VAT deduction is e-invoice-only since 1 Apr 2023, cost recognition since 1 Jul 2023, and the Law 6/2025 small-business regime requires enrollment in both systems. (Verified 2026-08-30 — see the addendum.) ServeOS has none of the machinery: no per-line tax codes, no taxpayer/device registration, no signing, no submission, no UUID/QR on the receipt, and no credit-note path for a refund. It also must not become a *blocking* dependency of the till — ETA e-receipts allow submission **within a window (up to ~24h) rather than synchronously** ([flick.network](https://www.flick.network/en-eg/egypt-b2c-e-receipt-mandate)), and a cash register that stops selling because a government API is slow is worse than useless.
...
| F4 | Submission timing | **Asynchronous, never blocks the sale.** `recordSale` commits, then enqueues; a scheduled worker submits with retry — reusing the Spec 5 outbox-worker pattern. **Confirmed official (2026-08-30):** receipts are post-clearance with a **24-hour** submission window plus a formal Late Submission Request path; the submission API itself is async (HTTP 202 + submissionUUID, then poll). See the addendum §3. |
| F5 | Refund → return receipt | A Spec 3 refund on a fiscalised sale enqueues a **`return_receipt`** submission (the e-receipt system's refund document) referencing the original receipt's UUID, allowed ≤ 540 days. `credit_note` remains for the deferred B2B `e_invoice` docType. **(Amended per addendum C4.)** |
...
**2026-08-30: five** new tenant-scoped tables — the three below **plus `eta_device_chains` and `eta_pos_credentials`** (per-device UUID chain state and per-device API credentials; see addendum §2). All (`FORCE ROW LEVEL SECURITY`, reads/writes through `withTenant`, `src/db/with-tenant.ts`), matching the canonical names in `docs/ROADMAP.md:107`. `eta_submissions` deliberately mirrors the Spec 5 `notification_outbox` shape (status + attempts + lastError) so the same store-and-forward worker semantics apply.
...
- **`apps/pos/src/screens/Receipt.tsx`**: render the **ETA QR + UUID at issuance** — the receipt UUID is self-computed and the QR is locally constructible (addendum C2/C5), and the printed customer copy must carry them at sale time. Submission status (`pending`/`submitted`/`accepted`/`rejected`) is a separate indicator; on `rejected` show a non-blocking notice (the fix is a corrected resubmission via `referenceOldUUID`, never a re-sale). **(Amended per addendum C5.)**
...
**2026-08-30 UPDATE: items 1–6 and 8 are now RESOLVED against official ETA sources; item 7 remains an operational per-tenant check.** See **`2026-08-30-eta-verified-findings-addendum.md`** (authoritative delta; verified against official ETA/ITIDA sources). The original items are preserved below for traceability.
